...
class intcode():
    ...
    def calc_addr(self,mode,param):
        pc = self.pc
        return self.data[pc+param] if mode[-param] == '0' else pc+param if mode[-param] == '1' else self.data[pc+param]+self.rb
    ...
    def step(self):
        ...
        #print(self.pc,instr,mode,str(self.data[pc]))

        if instr in [1,2,7,8]:
            addr1 = self.calc_addr(mode,1)
            addr2 = self.calc_addr(mode,2)
            addr3 = self.calc_addr(mode,3)
            self.ext_data_buffer(addr1)
            self.ext_data_buffer(addr2)
            self.ext_data_buffer(addr3)

            if instr == 1:
                dat[addr3] = dat[addr1] + dat[addr2]
            elif instr == 2:
                dat[addr3] = dat[addr1] * dat[addr2]
            elif instr == 7:
                dat[addr3] = 1 if dat[addr1] < dat[addr2] else 0
            elif instr == 8:
                dat[addr3] = 1 if dat[addr1] == dat[addr2] else 0
            pc += 4
            ...
            if instr == 3:
                if type(self.input) == int:
                    dat[addr1] = self.input
                else:
                    dat[addr1] = self.input[self.ic]
                    self.ic = (self.ic + 1) % len(self.input)
            elif instr == 4:
                self.output.append(dat[addr1])
            ...
            self.ext_data_buffer(addr2)
            if instr == 5:
                pc = pc+3 if dat[addr1] == 0 else dat[addr2]
            elif instr == 6:
                pc = dat[addr2] if dat[addr1] == 0 else pc+3
